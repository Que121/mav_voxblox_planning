...
#include <mgv_trajectory_generation/trajectory_sampling.h>
...
namespace mgv_planning
{
  ...
  MgvLocalPlanner::MgvLocalPlanner(const ros::NodeHandle &nh,
      ...
      : nh_(nh),
        ...
        avoid_collisions_(0),      //避障
        plan_to_startOFmgv_(1),    //开始规划
        path_indexOFmgv_(0)        //索引
  {
    //订阅机器人位置信息
  ...
  //回调  接收VO数据
  void MgvLocalPlanner::odometryCallback_mgv(const ngv_msgs::Odometry &msg)
  {
    // 消息类型需要更改
      ...
      const ros::TimerEvent &event)
  {
    ...
    if (path_indexOFmgv_ < path_queueOFmgv_.size())
    {
      ...
      //7.将 trajectory_to_publish 中的路径点转换为 ROS 的控制指令消息格式。
