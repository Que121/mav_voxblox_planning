...
namespace loco_planner
{
                                   ...
                                   size_t num_segments, double total_time)
  {
    mgv_trajectory_generation::Vertex::Vector vertices(
        num_segments + 1, mgv_trajectory_generation::Vertex(K_));

    vertices.front().makeStartOrEnd(
        0, mgv_trajectory_generation::getHighestDerivativeFromN(N));
    vertices.front().addConstraint(
        mgv_trajectory_generation::derivative_order::POSITION, start);
    vertices.back().makeStartOrEnd(
        0, mgv_trajectory_generation::getHighestDerivativeFromN(N));
    vertices.back().addConstraint(
        mgv_trajectory_generation::derivative_order::POSITION, goal);
  ...
  {
    mgv_trajectory_generation::timing::Timer timer_setup("loco/setup");
    ...
    vertices.emplace_back(trajectory.getStartVertex(
        mgv_trajectory_generation::getHighestDerivativeFromN(N)));
    for (size_t i = 0; i < times.size() - 1; ++i)
    {
      vertices.emplace_back(mgv_trajectory_generation::Vertex(K_));
    }
    ...
    if (config_.soft_goal_constraint)
    {
      vertices.back().getConstraint(
          mgv_trajectory_generation::derivative_order::POSITION, &goal_pos_);
      ...
      size_t num_segments)
  {
    ...
    {
      d_all = M_pinv_ * A_ * p[i];
      d_p[i] = d_all.tail(poly_opt_.getNumberFreertieConstraints());
    }
    poly_opt_.setFreeConstraints(d_p);
  ...
  void Loco<N>::solveProblem()
  {
    ...
    mgv_trajectory_generation::timing::Timer timer_solve("loco/solve");
    solveProblemCeres();
      ...
      std::vector<Eigen::VectorXd> *gradients) const
  {
    ...
    mgv_trajectory_generation::timing::Timer timer_cost_grad_d(
        "loco/cost_grad_d");
    ...
    mgv_trajectory_generation::timing::Timer timer_cost_grad_c(
        "loco/cost_grad_c");
    if (gradients != nullptr)
    {
    ...
    if (config_.soft_goal_constraint)
    {
      mgv_trajectory_generation::timing::Timer timer_cost_grad_g(
          "loco/cost_grad_g");
      if (gradients != nullptr)
      {
    ...
    if (!waypoints_.empty())
    {
      mgv_trajectory_generation::timing::Timer timer_cost_grad_w(
          "loco/cost_grad_w");
      if (gradients != nullptr)
      {
  ...
  double Loco<N>::computeCollisionCostAndGradient(
      std::vector<Eigen::VectorXd> *gradients) const
  {
    mgv_trajectory_generation::timing::Timer coll_cost_prep_timer(
        "loco/coll_cost_prep");
    ...
    for (int i = 0; i < num_segments; ++i)
    {
      ...
      {
        mgv_trajectory_generation::timing::Timer coll_cost_sample_timer(
        ...
        mgv_trajectory_generation::timing::Timer timer_map_lookup(
            "loco/map_lookup");
        double c = 0.0;
        if (gradients != nullptr)
        {
        ...
        mgv_trajectory_generation::timing::Timer coll_cost_grad_timer(
            "loco/coll_cost_grad");
  ...
  // 从轨迹点开始优化 11111111111111111111
  template <int N>
  void Loco<N>::setupFromTrajectoryPoints(
      const mgv_msgs::EigenTrajectoryPointMgv &start_point,
      const mgv_msgs::EigenTrajectoryPointMgv &goal_point,
      size_t num_segments,
      double total_time)
  {
    // 初始化顶点类的容器，数量 、维度
  ...
  // Vertices can be modified by this function. 222222222222222222222
  template <int N>
  void Loco<N>::setupFromVertices(
      double total_time, mgv_trajectory_generation::Vertex::Vector *vertices)
  {
    // 开启定时器
    mgv_trajectory_generation::timing::Timer timer_setup("loco/setup");
    ...
    // 进行时间优化
    poly_opt_.setupFromVertices(*vertices, times, config_.derivative_to_optimize); // mgv_trajectory_generation::PolynomialOptimization<N> poly_opt_;
  ...
  template <int N> // 333333333333333333333333333
